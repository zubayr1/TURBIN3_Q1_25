...
export function OneCsCreate() {
  const { initialize } = useOneCsProgram()
      ...
      {accounts.isLoading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {accounts.data?.map((account) => (
            <OneCsCard key={account.publicKey.toString()} account={account.publicKey} />
          ))}
        </div>
      ) : (
...
function OneCsCard({ account }: { account: PublicKey }) {
  const { accountQuery, incrementMutation, setMutation, decrementMutation, closeMutation } = useOneCsProgramAccount({
    account,
  })
